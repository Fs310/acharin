...
def profile(request):
    ...
    return render(request, 'accounts/Profile.html', {'profiles': profiles, 'form_user': form_user, 'form_profile': form_profile, 'list_order': list_order})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user:
                ...
                send_mail('بازیابی رمز عبور آچارین', f'برای تغییر رمز عبور از لینک زیر استفاده کنید:\n{reset_link}', settings.DEFAULT_FROM_EMAIL, [email], fail_silently=False)
...
def submit_review(request, factor_id, service_id):
    ...
    if request.method == 'POST':
        ...
        form = ReviewForm()
    return render(request, 'accounts/submit_review.html', {'form': form, 'factor': factor, 'service': service, 'factor_service': factor_service})
